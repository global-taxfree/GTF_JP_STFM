...
namespace GTF_STFM.Tran
{
    ...
    class Transaction
    {
        ...
        string url_ServerAlipayIDSearch = string.Empty; //알리페이 ID조회
        string url_ServerAliDataTran = string.Empty; //알리페이 서버전송
        ...
        public Transaction()
        {
            ...
            url_ServerAlipayIDSearch = "/service/jtc/alipayIDSearch"; //알리페이ID조회
            url_ServerAliDataTran = "/service/jtc/addAlipayConfirm"; //알리페이 정보 전송
            ...
            try
            {
                ...
                else
                {
                    ...
                    if ("S".Equals(jsonRes["login"].ToString()))
                    {
                        ...
                        Constants.SQLITE_MANAGER.DropTable("USERINFO".ToUpper()); //DB 삭제
                        Constants.SQLITE_MANAGER.createTable(user_info, "USERINFO".ToUpper());//DB생성                        
                ...
                if ("S".Equals((string)jsonRes["result"]))
                {
                    ...
                    for (int i = 0; i < arrJsonRes.Count; i++)
                    {
                        ...
                        //if (jsonWhere == null ) 
                        {
                            ...
                            if ("code".Equals(strCode_type) && Constants.SQLITE_MANAGER.ExitsTable("CODE"))
                            {
                                JObject jsonWhere = new JObject();
                                jsonWhere.Add("CODEDIV", tmpObj["CODEDIV"].ToString());
                                jsonWhere.Add("COMCODE", tmpObj["COMCODE"].ToString());
                                JArray arrRet = Constants.SQLITE_MANAGER.SelectTable(strCode_type.ToUpper(), jsonWhere);
                                if (arrRet != null && arrRet.Count > 0)
                                {
                                    Constants.SQLITE_MANAGER.updateTable(tmpObj, jsonWhere, strCode_type.ToUpper());
                                }
                                else
                                {
                            ...
                            else
                            {
                                if (i == 0)
                                {
                                    Constants.SQLITE_MANAGER.DropTable(strCode_type.ToUpper()); //DB 삭제
                                    Constants.SQLITE_MANAGER.createTable(tmpObj, strCode_type.ToUpper());//DB생성
                                }
        ...
        public JArray SearchShopList(JObject jsonWhere = null)
        {
            ...
            arrjsonRes = Constants.SQLITE_MANAGER.SelectTable("MERCHANT", jsonWhere);
            return arrjsonRes;
        }
        ...
        public JArray SearchAdsList(JObject jsonWhere = null)
        {
            ...
            JArray arrjsonRes = new JArray();
            //DB 조회
            arrjsonRes = Constants.SQLITE_MANAGER.SelectTable("ADS", jsonWhere);
            return arrjsonRes;
        }
        ...
        public JArray SearchContryList(JObject jsonWhere = null)
        {
            ...
            //DB 조회
            arrjsonTempRes = Constants.SQLITE_MANAGER.SelectTableOrderCounry("CODE", jsonWhere);
        ...
        public JArray SearchItemList(JObject jsonWhere = null)
        {
            ...
            JArray arrjsonTempRes = null;
            //로컬 DB 조회
            arrjsonTempRes = Constants.SQLITE_MANAGER.SelectTable("ITEM", jsonWhere);
            for (int i = 0; i < arrjsonTempRes.Count; i++)
            {
                //국적만 받아옴
        ...
        public long getSeq(string strKey)
        {
            ...
            JArray arrRet = Constants.SQLITE_MANAGER.SelectTable("ID_CREATION_RULE", jsonWhere);
            if (arrRet == null || arrRet.Count == 0)
            {
            ...
            else
            {
                ...
                Constants.SQLITE_MANAGER.updateTable(jsonUpdate, jsonWhere, "ID_CREATION_RULE");
        ...
        public Boolean InsertSlip(JObject jsonSlip, JArray arrItems, JObject jsonSlipDocs)
        {
            ...
            {
                //전표저장
                if (!Constants.SQLITE_MANAGER.ExitsTable("REFUNDSLIP"))
                {
                    Constants.SQLITE_MANAGER.createTable(jsonSlip, "REFUNDSLIP");//DB생성
                }

                if (!Constants.SQLITE_MANAGER.ExitsTable("SALES_GOODS"))
                {
                    if (arrItems != null && arrItems.Count > 0)
                        Constants.SQLITE_MANAGER.createTable((JObject)arrItems[0], "SALES_GOODS");//DB생성
                }

                if (!Constants.SQLITE_MANAGER.ExitsTable("SLIP_PRINT_DOCS"))
                {
                    Constants.SQLITE_MANAGER.createTable(jsonSlipDocs, "SLIP_PRINT_DOCS");//DB생성
                }
        ...
        public Boolean InsertSlipSign(JObject jsonSlipSing)
        {
            ...
            try
            {
                if (!Constants.SQLITE_MANAGER.ExitsTable("REFUND_SLIP_SIGN"))
                {
                    Constants.SQLITE_MANAGER.createTable(jsonSlipSing, "REFUND_SLIP_SIGN");//DB생성
                }
        ...
        public Boolean UpdateSlip(JObject jsonSlip, JObject jsonFlag)
        {
            ...
            try
            {
                Constants.SQLITE_MANAGER.updateTable(jsonFlag, jsonSlip, Constants.TABLE_REFUND_SLIP);
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex.Message);
        ...
        {
            JArray jsonRes = null;
            if (Constants.SQLITE_MANAGER.ExitsTable(Constants.TABLE_REFUND_SLIP))
            {
                ...
                jsonRes = Constants.SQLITE_MANAGER.getSlipList("", Constants.TABLE_REFUND_SLIP, strStartDate, strEndDate
        ...
        public JArray SearchUserSlips(string strStartDate, string strEndDate, string strSendFlag, string strSlipStatus = "", string strSlipNo = "")
        {
            ...
            if (Constants.SQLITE_MANAGER.ExitsTable(Constants.TABLE_REFUND_SLIP))
            {
                ...
                jsonRes = Constants.SQLITE_MANAGER.getSlipList(Constants.USER_ID, Constants.TABLE_REFUND_SLIP, strStartDate, strEndDate
                    , "ALL".Equals(strSendFlag.Trim()) ? "" : strSendFlag.Trim()
                    , "ALL".Equals(strSlipStatus.Trim()) ? "" : strSlipStatus.Trim()
                    , strSlipNo
                    );
        ...
        public JArray SearchSlipDetail(string strSlipNo)
        {
            ...
            arrJsonRes = Constants.SQLITE_MANAGER.SelectTable("SALES_GOODS", jsonWhere);
            jsonWhere.RemoveAll();
            return arrJsonRes;
        }
        ...
        public JArray getLastUserInfo()
        {
            //JObject jsonRes = null;
            JArray arrjsonRes = new JArray();
            arrjsonRes = Constants.SQLITE_MANAGER.SelectTable("USERINFO");
            return arrjsonRes;
        }
        ...
        public Boolean Upi_BinCHeck(string strCardNo)
        {
            ...
            JArray arrjsonRes = new JArray();
            arrjsonRes = Constants.SQLITE_MANAGER.SelectUpiBinTable("BIN", strCardNo);
        ...
        public string BuildSlipDoc(string strSlipNo)
        {
            ...
            //로컬DB 조회 결과
            JArray arrSlip = Constants.SQLITE_MANAGER.SelectTable("REFUNDSLIP", jsonWhere);
            JArray ArrSlipItems = Constants.SQLITE_MANAGER.SelectTable("SALES_GOODS", jsonWhere);
        ...
        public JObject GetPrintDocs(string strSlipNo)
        {
            ...
            JObject jsonWhere = new JObject();
            jsonWhere.Add("SLIP_NO", strSlipNo);
            JArray arrSlip = Constants.SQLITE_MANAGER.SelectTable("SLIP_PRINT_DOCS", jsonWhere);
        ...
        public string sendSlipSignData()
        {
            ...
            table_ex = Constants.SQLITE_MANAGER.ExitsTable("REFUND_SLIP_SIGN");
            ...
            if (table_ex)
            {
                ...
                JArray arrSlip = Constants.SQLITE_MANAGER.SelectTable("REFUND_SLIP_SIGN", jsonWhere);
                    ...
                    for (int j = 0; j < arrSlip.Count; j++)
                    {
                        ...
                        else
                        {
                            ...
                            if ("S".Equals((string)resultJson["result"]))
                            {
                                ...
                                Constants.SQLITE_MANAGER.updateTable(tmpObj, jsonSignWhere, "REFUND_SLIP_SIGN");
                            }
        ...
        //알리페이 ID조회
        public string AlipayQR(string jsonReq)
        {
            string strRes = string.Empty;
            JObject resultJson = new JObject();
            ...
            try
            {
                ...
                strRes = comm.sendHttp_Json(Constants.SERVER_URL + url_ServerAlipayIDSearch, jsonReq.ToString(), true);
        ...
        //알리페이 정보저장
        public Boolean InsertAlipayConfirm(JObject m_AlipayData)
        {
            Boolean bRet = true;
            try
            {
                if (!Constants.SQLITE_MANAGER.ExitsTable("ALIPAY_CONFIRM"))
                {
                    Constants.SQLITE_MANAGER.createTable(m_AlipayData, "ALIPAY_CONFIRM");//DB생성
                }

                Constants.SQLITE_MANAGER.InsertTable(m_AlipayData, "ALIPAY_CONFIRM", 2);
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex.Message);
        ...
        public string sendAlipayData()
        {
            ...
            JArray arrSlip = Constants.SQLITE_MANAGER.SelectTable("ALIPAY_CONFIRM", jsonWhere);
            Console.WriteLine("ALIPAY 전송대상[" + arrSlip.Count + "]");
            ...
            if (arrSlip.Count > 0)
            {
                ...
                {
                    jsonRet = (JObject)arrSlip[j];
                    Console.WriteLine("ALIPAY 전표번호[" + jsonRet["SLIP_NO"].ToString() + "]");

                    JObject SlipAli = new JObject();

                    SlipAli.Add("slip_no", jsonRet["SLIP_NO"].ToString());
                    SlipAli.Add("alipay_user_id", jsonRet["ALIPAY_ID"].ToString());
                    SlipAli.Add("user_name", jsonRet["USER_NAME"].ToString());
                    SlipAli.Add("alipay_barcode_no", jsonRet["ALIPAY_BARCODE_NO"].ToString());
                    SlipAli.Add("alipay_login_id", jsonRet["ALIPAY_LOGIN_ID"].ToString());
                    SlipAli.Add("out_order_no", jsonRet["OUT_ORDER_NO"].ToString());
                    SlipAli.Add("confirm_date", jsonRet["CONFIRM_DATE"].ToString());
                    SlipAli.Add("confirm_time", jsonRet["CONFIRM_TIME"].ToString());
                    SlipAli.Add("userId", Constants.USER_ID);

                    strReq = SlipAli.ToString();

                    string strRes = comm.sendHttp_Json(Constants.SERVER_URL + url_ServerAliDataTran, strReq, true);
                    ...
                    else
                    {
                        ...
                        if ("S".Equals((string)resultJson["result"]))
                        {
                            ...
                            Constants.SQLITE_MANAGER.updateTable(tmpObj, jsonSignWhere, "ALIPAY_CONFIRM");
                        }
