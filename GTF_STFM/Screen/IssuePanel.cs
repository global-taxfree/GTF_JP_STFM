...
namespace GTF_STFM.Screen
{
    public partial class IssuePanel : UserControl
    {
        ...
        private void IssuePanel_Load(object sender, EventArgs e)
        {
            ...
            COM_REFUND_TYPE.Items.Add(new Utils.ComboItem("03", Constants.getScreenText("COMBO_ITEM_ALI")));
        ...
        private void BTN_SUBMIT_Click(object sender, EventArgs e)
        {
            try
            {
               //처리중에 이중 입력 방지
                ...
                else if (((Utils.ComboItem)COM_REFUND_TYPE.SelectedItem).Value.Equals("03"))//Alipay 환급방법
                {
                    if (TXT_REFUND_TYPE_KEY.Text == null || string.Empty.Equals(TXT_REFUND_TYPE_KEY.Text.ToString().Trim()))
                    {
                        MetroMessageBox.Show(this, Constants.getMessage("ERROR_ALI_EMPTY"), "Issue", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        TXT_REFUND_TYPE_KEY.Focus();
                        TXT_REFUND_TYPE_KEY.SelectAll();
                        setWaitCursor(false);
                        return;
                ...
                if (Constants.SIGNPAD_USE == "YES")
                {
                    ...
                    {
                        MetroMessageBox.Show(this, "お客様の署名を再確認してください", "Issue", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        setWaitCursor(false);
                        return;
                    }
                }
                ...
                try
                {
                    ...
                    tourist += "|" + TXT_PERMIT_NO.Text.ToString();//체재유형
                    ...
                    //for (int j = 0; j < arrShop.Count; j++)
                    {
                        ...
                        //docid += "|" + Constants.SLIP_TYPE;         //출력전표 갯수
                        /*
                        if ("07".Equals(strOutputSlipType) )
                        {
                            docid += "|" + 2;           //출력전표 갯수
                            docid += "|" + "01/02";     //출력
                        }
                        else if ("08".Equals(strOutputSlipType))
                        {
                            docid += "|" + 3;           //출력전표 갯수
                            docid += "|" + "01/02/04";  //출력
                        }
                        ...
                        {
                            docid += "|" + arrOutputSlipType.Length;    //출력전표 갯수
                            docid += "|" + strOutputSlipType;
                        }
                        */
                        ...
                        else
                        {
                            ...
                            else if (((Utils.ComboItem)COM_REFUND_TYPE.SelectedItem).Value.Equals("03"))//ALIPAY 번호 마스킹
                            {
                                docid += "|" + m_AlipayData.GetValue("alipay_login_id").ToString();
                            }
                            else
                    ...
                    for (int i = 0; i < arrSlips.Count; i++)
                    {
                        ...
                        if ("03".Equals(((Utils.ComboItem)COM_REFUND_TYPE.SelectedItem).Value.ToString()))
                        {

                            JObject tmpAli = new JObject();
                            tmpAli.Add("SLIP_NO", tmpSlip["SLIP_NO"]);
                            tmpAli.Add("ALIPAY_ID", m_AlipayData.GetValue("alipay_id"));
                            tmpAli.Add("USER_NAME", m_AlipayData.GetValue("user_name"));
                            tmpAli.Add("ALIPAY_BARCODE_NO", m_AlipayData.GetValue("alipay_barcode_no"));
                            tmpAli.Add("ALIPAY_LOGIN_ID", m_AlipayData.GetValue("alipay_login_id"));
                            tmpAli.Add("OUT_ORDER_NO", m_AlipayData.GetValue("out_order_no"));
                            tmpAli.Add("CONFIRM_DATE", m_AlipayData.GetValue("confirm_date"));
                            tmpAli.Add("CONFIRM_TIME", m_AlipayData.GetValue("confirm_time"));
                            tmpAli.Add("SEND_YN", "N");
                            tran.InsertAlipayConfirm(tmpAli);
                        ...
                        if (insert_check)  //------------------------------
                        {
                            ...
                            if ((tmpCUSTOM_SEND_FLAG.Equals("Y")) && (Constants.PRINT_SETTING == "1")) 
                                ...
                                printer.PrintSlip_ja(tmpDocid.Replace("[REPUBLISH]", "0"), tmpRetailer, tmpGoods, tmpTourist, tmpDdsinfo, "Y".Equals(strTmpPreviewUse), tmpSignInfo);
                    ...
                    //if (msgForm.DialogResult == DialogResult.Yes)
                    {
                        ...
                        //Alipay
                        else if ("03".Equals(((Utils.ComboItem)COM_REFUND_TYPE.SelectedItem).Value.ToString()))
                        {
                            MessageForm msgCompleteForm = new MessageForm(Constants.getMessage("REFUND_Alipay") + " : " + nSlipTotalRefundAmt, null);
                            msgCompleteForm.ShowDialog(this);
                        }
                        //전표상태업데이트
                        for (int i = 0; i < arrSlips.Count; i++)
        ...
        private void COM_REFUND_TYPE_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (((Utils.ComboItem)COM_REFUND_TYPE.SelectedItem).Value.Equals("04"))//카드 환급방법
            {
                ...
                TXT_REFUND_TYPE_KEY.ReadOnly = false;
                TXT_REFUND_TYPE_KEY.Focus();
            }
            ...
            else if (((Utils.ComboItem)COM_REFUND_TYPE.SelectedItem).Value.Equals("03"))//ALIPAY REFUND
            {
                /* Test를 위해 막아둠
                if (GRD_ITEMS.Rows.Count < 1)
                {
                    MetroMessageBox.Show(this, Constants.getMessage("ERROR_ALI_INPUT"), "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                ...
                */
                ...
                TXT_REFUND_TYPE_KEY.Enabled = true;
                TXT_REFUND_TYPE_KEY.Visible = true;
                TXT_REFUND_TYPE_KEY.ReadOnly = true;
                TXT_REFUND_TYPE_KEY.Focus();
                ...
                string alipayID = ALIPAY_ID_SEARCH();

                TXT_REFUND_TYPE_KEY.Text = alipayID.ToString();
                setWaitCursor(false);
        ...
        {
            //여권 정보로 발행을 시도한 경우, 데이터 필수값 미입력 체크하기.
            if (bPassPort)
            {
                ...
                //재류자격 코드가 여권으로 되어 있는지 (허가서 관련이 아닌지 체크하는 부분을 추가해야 함)
            }
            else
            {
        ...
        private void SCREEN_CLEAR()
        {
            ...
            birth_validate = false;
            expire_validate = false;
            BTN_SCAN.Focus();
        }
        ...
        private String ALIPAY_ID_SEARCH()
        {
            string alipay_id = "";
            ScanAli scanAli = new ScanAli();
            scanAli.ShowDialog(this);

            alipay_id = scanAli.alipayID;

            m_AlipayData = new JObject();
            m_AlipayData.Add("alipay_id", alipay_id);
            m_AlipayData.Add("user_name", scanAli.user_name);
            m_AlipayData.Add("alipay_barcode_no", scanAli.alipay_barcode_no);
            m_AlipayData.Add("alipay_login_id", scanAli.alipay_login_id);
            m_AlipayData.Add("out_order_no", scanAli.out_order_no);
            m_AlipayData.Add("confirm_date", scanAli.confirm_date);
            m_AlipayData.Add("confirm_time", scanAli.confirm_time);
        ...
        private void TXT_RCT_NO_Click(object sender, EventArgs e)
        {
