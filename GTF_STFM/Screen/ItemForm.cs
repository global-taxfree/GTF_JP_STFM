        ...
        */
        ...
        public void Init(int nGoodsCount, int nConsumsCount)
        {
            try {
                ...
                if (arrRet.Count > 0)
                {
                    ...
                    m_Fee_8 = decimal.Parse(Constants.FEE_8_RATE);                    //수수료 비율
                    m_Fee_10 = decimal.Parse(Constants.FEE_10_RATE);                    //수수료 비율
        ...
        private void CAL_AMOUNT(DataGridViewCellEventArgs ev = null, MetroFramework.Controls.MetroGrid ctlGrd = null)
        {
            ...
            for (int i = 0; i < GRD_GOODS.RowCount; i++)
            {
                ...
                //환급액
                if (GRD_GOODS[9, i].Value != null &&  GRD_GOODS[9, i].Value.ToString().Equals("1"))
                {
                    tempAmt = (Convert.ToInt64((goodsTax)) * m_Fee_8);
                }
                else if (GRD_GOODS[9, i].Value != null && GRD_GOODS[9, i].Value.ToString().Equals("2"))
                {
                    tempAmt = (Convert.ToInt64((goodsTax)) * m_Fee_10);
                }
                else
                {
                    tempAmt = (Convert.ToInt64((goodsTax)) * m_Fee_10);
            ...
            for (int i = 0; i < GRD_CONSUM.RowCount; i++)
            {
                ...
                //환급액
                if (GRD_CONSUM[9, i].Value != null &&  GRD_CONSUM[9, i].Value.ToString().Equals("1"))
                {
                    tempAmt = (Convert.ToInt64((consumTax)) * m_Fee_8);
                }
                else if (GRD_CONSUM[9, i].Value != null && GRD_CONSUM[9, i].Value.ToString().Equals("2"))
                {
                    tempAmt = (Convert.ToInt64((consumTax)) * m_Fee_10);
                }
                else
                {
                    tempAmt = (Convert.ToInt64((consumTax)) * m_Fee_10);
        ...
        private void GRD_CONSUM_DataError(object sender, DataGridViewDataErrorEventArgs e)
        {
            Console.WriteLine("GRD_CONSUM_DataError");
        }
