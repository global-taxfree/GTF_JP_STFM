            ...
            try {
                ...
                else
                {
                    ...
                    for (int i = 0; i < arrRet.Count; i++)
                    {
                        ...
                        GRD_SLIP[3, i].Value = tempObj["SALEDT"].ToString();
                        GRD_SLIP[4, i].Value = tempObj["SHOP_NAME"].ToString();
                        GRD_SLIP[5, i].Value = Int64.Parse(tempObj["GOODS_BUY_AMT"].ToString());
                        GRD_SLIP[6, i].Value = Int64.Parse(tempObj["GOODS_TAX_AMT"].ToString());
                        GRD_SLIP[7, i].Value = Int64.Parse(tempObj["GOODS_REFUND_AMT"].ToString());
                        GRD_SLIP[8, i].Value = Int64.Parse(tempObj["CONSUMS_BUY_AMT"].ToString());
                        GRD_SLIP[9, i].Value = Int64.Parse(tempObj["CONSUMS_TAX_AMT"].ToString());
                        GRD_SLIP[10, i].Value = Int64.Parse(tempObj["CONSUMS_REFUND_AMT"].ToString());
                        GRD_SLIP[11, i].Value = "02".Equals(tempObj["SLIP_STATUS_CODE"].ToString()) ? Constants.getScreenText("COMBO_ITEM_REFUND") : Constants.getScreenText("COMBO_ITEM_UNREFUND");
                        if (tempObj.GetValue(Constants.PRINT_CNT) == null || string.Empty.Equals(tempObj[Constants.PRINT_CNT].ToString().Trim()))
                        {
                            GRD_SLIP[12, i].Value = 0;
                        }
                        else
                        {
                            GRD_SLIP[12, i].Value = Int64.Parse(tempObj[Constants.PRINT_CNT].ToString());
                        }
                        GRD_SLIP[13, i].Value = tempObj["SLIP_STATUS_CODE"].ToString();
                        tempObj = null;
                    }
                }
                tran = null;
