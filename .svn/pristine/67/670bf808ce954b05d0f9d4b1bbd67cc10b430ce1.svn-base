...
namespace GTF_Printer.GTF_JP
{
    partial class GTF_JPETRS
    {
        ...
        public int JPNPrintTicket(string docid, string retailer, string goods, string tourist, string adsinfo, string signinfo = null)
        {
            m_docid = docid;
            m_retailer = retailer;
            m_tourist = tourist;
        ...
        public void PrintRefundDetails(PrintPageEventArgs e, ref float yPos)
        {
            try
            {
                ...
                else if (MapRefundInfo[RefundWayInfo.REFUND_WAY_CODE].Equals("03"))
                {
                    e.Graphics.DrawString(String.Format("{0} : {1}", "", MapRefundInfo[RefundWayInfo.MASK_REMIT_NO]), printFont, Brushes.Black, 110, yPos, stringFormat);
                    yPos += printFont.GetHeight(e.Graphics);
                }
                else
                {
